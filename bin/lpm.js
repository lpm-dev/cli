...
const setupCommand = program
  .command("setup")
  .description("Configure .npmrc for CI or local development")
  .action(() => setupCommand.help());
setupCommand
  .command("ci")
  .description("Generate .npmrc for CI/CD")
  .option("-r, --registry <url>", "Custom registry URL")
  .option("--oidc", "Exchange OIDC token")
  .option("--proxy", "Route all npm traffic through lpm.dev")
  .option("--scoped", "Scoped registry only")
  .action(setup);
setupCommand
  .command("local")
  .description("Generate read-only .npmrc token for local development")
