...
async function tryRefresh() {
	try {
		...
		const resp = await fetch(`${registryUrl}/api/cli/refresh`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ refreshToken, deviceFingerprint: fp }),
		})
...
export async function request(path, options = {}) {
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		...
		try {
			...
			// Handle authentication errors — try silent refresh before failing
			// Exception: 2FA OTP responses must be returned to the caller
			if (response.status === 401) {
				...
				if (data.code === "OTP_REQUIRED" || data.code === "OTP_INVALID") {
					return response
				}

				// Attempt silent refresh (Feature 44 Part B) — one retry on 401
				if (attempt === 0) {
					const refreshed = await tryRefresh()
					if (refreshed) {
						// Update the auth header and retry this request
			...
			if (response.status === 403) {
				...
				const data = await cloned403.json().catch(() => ({}))

				// 2FA enforcement responses must be returned to the caller
				if (data.code === "ORG_2FA_REQUIRED") {
					return response
				}
