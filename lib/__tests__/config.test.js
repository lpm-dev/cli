...
describe("config.js – getToken()", () => {
	...
	const originalRegistry = process.env.LPM_REGISTRY_URL

	beforeEach(() => {
		// Set a known registry so getRegistryUrl() doesn't go through Conf mock
		process.env.LPM_REGISTRY_URL = "https://lpm.dev"
	})
	...
	afterEach(() => {
		...
		if (originalRegistry === undefined) {
			...
			process.env.LPM_REGISTRY_URL = originalRegistry
		}
		vi.clearAllMocks()
	})
	...
	it("returns token from secure store", async () => {
		...
		expect(secureStoreMock.getToken).toHaveBeenCalledWith("https://lpm.dev")
	})
	...
	it("migrates legacy token from conf to secure store", async () => {
		...
		expect(secureStoreMock.setToken).toHaveBeenCalledWith(
			"legacy-token-789",
			"https://lpm.dev",
		)
...
describe("config.js – setToken()", () => {
	...
	it("clears token when null passed", async () => {
		await setToken(null)
		expect(secureStoreMock.clearToken).toHaveBeenCalledWith("https://lpm.dev")
		expect(mockConfInstance.delete).toHaveBeenCalledWith("token")
	})

	it("scopes token to current registry URL", async () => {
		process.env.LPM_REGISTRY_URL = "http://localhost:3000"
		await setToken("dev-token")
		expect(secureStoreMock.setToken).toHaveBeenCalledWith(
			"dev-token",
			"http://localhost:3000",
		)
...
describe("config.js – getAllConfig()", () => {
	...
	it("returns full config object", async () => {
		...
		expect(all.registryUrl).toBe("https://lpm.dev")
	})
})
