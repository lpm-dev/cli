...
const {
	...
	mockDetectFramework,
	mockGetDefaultPath,
	mockValidateComponentPath,
	mockValidateTarballPaths,
	mockGetProjectAliases,
	mockGetUserImportPrefix,
	mockResolveAliasForDirectory,
	mockIsSwiftProject,
	mockGetSpmTargets,
	mockExpandSrcGlob,
	mockFilterFiles,
	mockResolveConditionalDependencies,
	mockClackSelect,
	mockClackText,
	mockClackConfirm,
	mockClackIsCancel,
	mockExecAsync: _mockExecAsync,
} = vi.hoisted(() => {
	const spinner = {
		start: vi.fn(),
		stop: vi.fn(),
	...
	return {
		...
		mockResolveConditionalDependencies: vi
			.fn()
			.mockReturnValue({ npm: [], lpm: [] }),
...
vi.mock("../../swift-project.js", () => ({
	ensureXcodeLocalPackage: vi
		.fn()
		.mockReturnValue({ created: false, installPath: "/tmp/swift" }),
...
function setupSuccessfulDownload(opts = {}) {
	...
	const _origMkdtempSync = fs.mkdtempSync
...
describe("add command - --yes mode", () => {
	beforeEach(() => {
		...
		mockValidateComponentPath.mockReturnValue({
			valid: true,
			resolvedPath: "/tmp/test-target",
		})
	})
...
describe("add command - --dry-run", () => {
	...
	it("--dry-run + --json returns machine-readable output with dryRun field", async () => {
		...
		const writeCall = process.stdout.write.mock.calls.find(
			call => typeof call[0] === "string" && call[0].includes('"dryRun"'),
		)
		expect(writeCall).toBeDefined()
		const output = JSON.parse(writeCall[0])
	...
	it("--dry-run returns correct target directory", async () => {
		...
		mockValidateComponentPath.mockReturnValue({
			valid: true,
			resolvedPath: "/my/project/components",
		})

		await add("@lpm.dev/test.button", { json: true, dryRun: true })

		const writeCall = process.stdout.write.mock.calls.find(
			call => typeof call[0] === "string" && call[0].includes('"installPath"'),
		)
		const output = JSON.parse(writeCall[0])
