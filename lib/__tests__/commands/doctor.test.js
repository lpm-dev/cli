...
describe('doctor command', () => {
	...
	it('handles registry unreachable', async () => {
		...
		mockFetch.mockResolvedValueOnce({
			ok: true,
			json: () => Promise.resolve({ plan_tier: 'free', usage: { storage_bytes: 0, private_packages: 0 }, limits: {} }),
		})
	...
	it('detects .npmrc configuration', async () => {
		...
		mockFetch.mockResolvedValueOnce({
			ok: true,
			json: () => Promise.resolve({ plan_tier: 'pro', usage: { storage_bytes: 0, private_packages: 0 }, limits: {} }),
		})

		tmpDir = mkdtempSync(join(tmpdir(), 'lpm-doctor-'))
		writeFileSync(join(tmpDir, '.npmrc'), 'registry=https://lpm.dev/api/registry')
		vi.spyOn(process, 'cwd').mockReturnValue(tmpDir)
