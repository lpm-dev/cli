...
describe('marketplace compare command', () => {
	...
	it('exits with error for empty input', async () => {
		await expect(marketplaceCompare('', { json: true })).rejects.toThrow('process.exit')
	...
	it('handles API errors', async () => {
		mockGet.mockResolvedValueOnce({
			ok: false,
			status: 400,
			json: () => Promise.resolve({ error: 'At least one of "category" or "q" query parameter is required' }),
		})

		await expect(marketplaceCompare('some-input', { json: true })).rejects.toThrow('process.exit')
	...
	it('handles network errors', async () => {
		...
		await expect(marketplaceCompare('ui-components', { json: true })).rejects.toThrow('process.exit')
