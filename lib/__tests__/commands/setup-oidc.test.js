...
describe("lpm setup ci --oidc", () => {
  ...
  it("writes real OIDC token to .npmrc in scoped mode when exchange succeeds", async () => {
  ...
  it("mentions lpm setup local in default mode help text", async () => {
    tmpDir = mkdtempSync(join(tmpdir(), "lpm-oidc-"));
    vi.spyOn(process, "cwd").mockReturnValue(tmpDir);
    ...
    expect(log.info).toHaveBeenCalledWith(
      expect.stringContaining("lpm setup local"),
    );
