...
const { mockGetToken, mockGetRegistryUrl, mockFetch, mockSpinner, mockParseLpmPackageReference } = vi.hoisted(() => {
	const spinner = {
		start: vi.fn(),
		stop: vi.fn(),
		succeed: vi.fn(),
...
vi.mock('../../safe-path.js', () => ({
	validateComponentPath: vi.fn().mockReturnValue({ valid: true, resolvedPath: '/tmp/test' }),
...
describe('add command', () => {
	...
	it('fails when not logged in', async () => {
		...
		expect(mockSpinner.fail).toHaveBeenCalledWith(expect.stringContaining('Not logged in'))
	})
	...
	it('handles unauthorized access (403)', async () => {
		...
		await add('@lpm.dev/test.button', {})
		expect(mockSpinner.fail).toHaveBeenCalledWith(expect.stringContaining('Unauthorized'))
	})
	...
	it('handles version not found', async () => {
		...
		mockFetch.mockResolvedValueOnce({
			...
			json: () => Promise.resolve({
				...
				versions: { '1.0.0': { dist: { tarball: 'https://example.com/t.tgz' } } },
			}),
		})
