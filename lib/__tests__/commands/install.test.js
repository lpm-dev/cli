...
describe("install command", () => {
	...
	it("uses pnpm when --pm pnpm is passed", async () => {
		...
		await install(["@lpm.dev/test.pkg"], { pm: "pnpm" })

		expect(mockGetPackageManager).toHaveBeenCalledWith("pnpm")
		expect(mockSpawn).toHaveBeenCalledWith(
			"pnpm",
			expect.arrayContaining(["install", "@lpm.dev/test.pkg"]),
			expect.any(Object),
		)
	})

	it("uses yarn when --pm yarn is passed", async () => {
		tmpDir = mkdtempSync(join(tmpdir(), "lpm-install-"))
		vi.spyOn(process, "cwd").mockReturnValue(tmpDir)
		mockGetPackageManager.mockReturnValue("yarn")

		const child = new EventEmitter()
		mockSpawn.mockReturnValue(child)

		await install(["@lpm.dev/test.pkg"], { pm: "yarn" })
	...
	it("defaults to npm when no --pm flag", async () => {
		tmpDir = mkdtempSync(join(tmpdir(), "lpm-install-"))
		vi.spyOn(process, "cwd").mockReturnValue(tmpDir)
		...
		expect(mockGetPackageManager).toHaveBeenCalledWith(undefined)
		expect(mockSpawn).toHaveBeenCalledWith(
			"npm",
			expect.arrayContaining(["install"]),
			expect.any(Object),
