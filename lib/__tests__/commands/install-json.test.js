...
describe('install command - --json mode', () => {
	...
	it('--json writes structured JSON on auth failure', async () => {
		...
		expect(output.errors).toContain('You must be logged in to install packages. Run "lpm login" first.')
	})
	...
	it('--json captures npm error on failure', async () => {
		...
		// close with code 1 calls process.exit which throws
		expect(() => child.emit('close', 1)).toThrow('process.exit')
