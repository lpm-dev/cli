...
const MOCK_QUALITY_RESPONSE = {
	...
	checks: [
		{ id: 'has-readme', category: 'documentation', label: 'Has README', passed: true, points: 8, maxPoints: 8, detail: 'README found (2345 chars)' },
		{ id: 'has-types', category: 'code', label: 'Has Types', passed: true, points: 6, maxPoints: 6, detail: 'TypeScript types found' },
		{ id: 'has-test-files', category: 'testing', label: 'Has Tests', passed: false, points: 0, maxPoints: 7, detail: 'No test files found' },
	],
...
describe('quality command', () => {
	...
	it('exits with error for invalid format', async () => {
		await expect(quality('no-dot', { json: true })).rejects.toThrow('process.exit')
		expect(mockExit).toHaveBeenCalledWith(1)
	})
	...
	it('handles 404 errors', async () => {
		...
		await expect(quality('alice.no-exist', { json: true })).rejects.toThrow('process.exit')
	...
	it('handles 403 errors for private packages', async () => {
		mockGet.mockResolvedValueOnce({
			ok: false,
			status: 403,
			json: () => Promise.resolve({ error: 'Authentication required for private packages' }),
		})

		await expect(quality('alice.private-pkg', { json: true })).rejects.toThrow('process.exit')
	...
	it('handles network errors gracefully', async () => {
		...
		await expect(quality('alice.my-utils', { json: true })).rejects.toThrow('process.exit')
