...
describe("check-name command", () => {
	...
	it("exits with error for invalid format (no dot)", async () => {
		await expect(checkName("no-dot-here", { json: true })).rejects.toThrow(
			"process.exit",
		)
		expect(mockExit).toHaveBeenCalledWith(1)
	...
	it("exits with error for name starting with dot", async () => {
		await expect(checkName(".leading-dot", { json: true })).rejects.toThrow(
			"process.exit",
		)
		expect(mockExit).toHaveBeenCalledWith(1)
	})
	...
	it("exits with error for name ending with dot", async () => {
		await expect(checkName("trailing.", { json: true })).rejects.toThrow(
			"process.exit",
		)
		expect(mockExit).toHaveBeenCalledWith(1)
	})
	...
	it("handles API errors gracefully", async () => {
		...
		await expect(checkName("alice.my-utils", { json: true })).rejects.toThrow(
			"process.exit",
		)
		expect(mockExit).toHaveBeenCalledWith(1)
