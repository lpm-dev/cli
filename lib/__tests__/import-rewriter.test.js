...
import { resolveImportToFilePath, rewriteImports } from "../import-rewriter.js"
...
describe("resolveImportToFilePath", () => {
	...
	it("resolves relative import to directory index", () => {
		const result = resolveImportToFilePath("./dialog", "components", fileSet)
...
describe("rewriteImports", () => {
	...
	it("rewrites relative imports to buyer alias", () => {
		...
		expect(result).toContain('from "@/components/design-system/lib/utils"')
	...
	it("handles side-effect imports", () => {
		...
		expect(result).toContain("@/ui/components/dialog/Dialog.style")
	})
