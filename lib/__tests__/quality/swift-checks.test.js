...
describe('Swift small-deps', () => {
	it('returns 5 points for 0 dependencies', () => {
	...
	it('returns 4 points for 2 dependencies', () => {
		const result = runCheck('small-deps', {
			swiftManifest: { dependencies: [{}, {}] },
		})
		expect(result.points).toBe(4)
	...
	it('returns 3 points for 5 dependencies', () => {
		const result = runCheck('small-deps', {
	...
	it('returns 5 points with no manifest', () => {
...
describe('Swift check registry', () => {
	it('has 24 checks total', () => {
		expect(swiftChecks).toHaveLength(24)
	})
	...
	it('includes universal has-ci-config check', () => {
		const check = swiftChecks.find(c => c.id === 'has-ci-config')
		expect(check).toBeDefined()
	})
