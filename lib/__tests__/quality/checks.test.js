...
describe('small-deps', () => {
	...
	it('returns 3 points for 3 dependencies', () => {
	...
	it('returns 2 points for 7 dependencies', () => {
	...
	it('returns 1 point for 15 dependencies', () => {
		const deps = Object.fromEntries(
			Array.from({ length: 15 }, (_, i) => [`dep${i}`, '1']),
		)
		const result = runCheck('small-deps', {
			packageJson: { dependencies: deps },
		})
	...
	it('returns 0 points for 16+ dependencies', () => {
		const deps = Object.fromEntries(
			Array.from({ length: 16 }, (_, i) => [`dep${i}`, '1']),
		)
		const result = runCheck('small-deps', {
			packageJson: { dependencies: deps },
		})
...
describe('has-test-files', () => {
	...
	it('passes with .spec.js files', () => {
		const result = runCheck('has-test-files', {
			files: [{ path: 'src/utils.spec.js' }],
		})
		expect(result.passed).toBe(true)
	})

	it('passes with __tests__ directory', () => {
		const result = runCheck('has-test-files', {
			files: [{ path: '__tests__/utils.js' }],
		})
		expect(result.passed).toBe(true)
	})

	it('fails without test files', () => {
		const result = runCheck('has-test-files', {
			files: [{ path: 'src/index.js' }],
		})
		expect(result.passed).toBe(false)
...
describe('has-ci-config', () => {
	it('passes with GitHub Actions workflow', () => {
		const result = runCheck('has-ci-config', {
			files: [{ path: '.github/workflows/ci.yml' }],
		})
		expect(result.passed).toBe(true)
	})

	it('fails without CI config', () => {
		const result = runCheck('has-ci-config', {
			files: [{ path: 'src/index.js' }],
		})
		expect(result.passed).toBe(false)
...
describe('getSourcePackageInfo', () => {
	...
	it('returns info for config with schema', () => {
		const result = getSourcePackageInfo({
			configSchema: { component: { type: 'select' }, theme: { type: 'boolean' } },
