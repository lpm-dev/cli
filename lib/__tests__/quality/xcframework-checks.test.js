...
describe("xcf-multi-slice", () => {
	it("returns 10 points for 4+ unique platforms", () => {
	...
	it("returns 8 points for 3 unique platforms", () => {
		const result = runCheck("xcf-multi-slice", {
			xcframeworkMeta: {
				slices: [
					{ platform: "ios", architectures: ["arm64"] },
	...
	it("returns 5 points for 2 unique platforms", () => {
		const result = runCheck("xcf-multi-slice", {
			xcframeworkMeta: {
				slices: [
					{ platform: "ios", architectures: ["arm64"] },
	...
	it("counts variant as separate platform", () => {
		...
		expect(result.points).toBe(5) // 2 unique: ios, ios-simulator
	})
...
describe("XCFramework check registry", () => {
	it("has 20 checks total", () => {
		expect(xcframeworkChecks).toHaveLength(20)
	})
	...
	it("category points sum to 100", () => {
		...
		for (const check of xcframeworkChecks) {
			catSums[check.category] =
				(catSums[check.category] || 0) + check.maxPoints
		}
		const total = Object.values(catSums).reduce((a, b) => a + b, 0)
		expect(total).toBe(100)
	})

	it("has documentation category = 25 points", () => {
	...
	it("has health category = 30 points", () => {
		const healthChecks = xcframeworkChecks.filter(
			(c) => c.category === "health",
		)
	...
	it("includes CI config as the only testing check", () => {
		const testingChecks = xcframeworkChecks.filter(
			(c) => c.category === "testing",
		)
		expect(testingChecks.length).toBeGreaterThanOrEqual(1)
		const ids = testingChecks.map((c) => c.id)
