...
vi.mock('chalk', () => {
	const p = str => str
	p.bold = p; p.green = p; p.red = p; p.cyan = p; p.gray = p; p.dim = p; p.blue = p; p.yellow = p; p.white = p
	return { default: p }
})
...
describe('quality/display.js – displayQualityReport()', () => {
	...
	it('displays score, tier, and category bars', () => {
		displayQualityReport({
			score: 72,
			checks: [
				{ id: 'has-readme', category: 'documentation', label: 'Has README', passed: true, points: 8, maxPoints: 8 },
				{ id: 'has-types', category: 'code', label: 'Has TypeScript types', passed: false, points: 0, maxPoints: 6 },
			],
			meta: {
				tier: 'good',
				score: 72,
	...
	it('shows pass/fail icons for checks', () => {
		displayQualityReport({
			...
			checks: [
				{ id: 'has-readme', category: 'documentation', label: 'Has README', passed: true, points: 8, maxPoints: 8 },
				{ id: 'has-types', category: 'code', label: 'Has types', passed: false, points: 0, maxPoints: 6 },
			],
			meta: {
				tier: 'fair',
				score: 50,
	...
	it('shows tips for failed checks', () => {
		displayQualityReport({
			score: 30,
			checks: [
				{ id: 'has-readme', category: 'documentation', label: 'Has README', passed: false, points: 0, maxPoints: 8 },
				{ id: 'has-types', category: 'code', label: 'Has types', passed: false, points: 0, maxPoints: 6 },
			],
			meta: {
				tier: 'needs-work',
	...
	it('shows server-only checks dimmed', () => {
		displayQualityReport({
			score: 60,
			checks: [
				{ id: 'no-eval', category: 'code', label: 'No eval', passed: true, points: 3, maxPoints: 3, serverOnly: true },
			],
			meta: {
				tier: 'fair',
				score: 60,
