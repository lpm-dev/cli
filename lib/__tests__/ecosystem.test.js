...
describe("detectXCFramework", () => {
	...
	it("detects .xcframework with Info.plist and parses slices", () => {
		...
		fs.mkdirSync(xcfDir)
		fs.writeFileSync(path.join(xcfDir, "Info.plist"), SAMPLE_PLIST)
...
describe("collectPackageFiles", () => {
	...
	it("collects all files in a directory", () => {
		...
		fs.writeFileSync(
			path.join(tmpDir, "Sources", "MyLib.swift"),
			"struct MyLib {}",
		)
	...
	it("skips .build directory for Swift", () => {
		...
		fs.writeFileSync(
			path.join(tmpDir, ".build", "debug.yaml"),
			"build artifact",
		)
...
// ============================================================================
// createEcosystemTarball — package/ prefix for strip:1 compatibility
// ============================================================================

describe("createEcosystemTarball", () => {
	let tmpDir

	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(process.cwd(), ".test-tarball-"))
	})
	...
	it("creates tarball with package/ prefix matching npm convention", async () => {
		// Set up a minimal Swift project
		fs.writeFileSync(
		...
		fs.mkdirSync(path.join(tmpDir, "Sources", "Haptic"), { recursive: true })
		fs.writeFileSync(
			path.join(tmpDir, "Sources", "Haptic", "Haptic.swift"),
			"struct Haptic {}",
		)

		const result = await createEcosystemTarball(
			"swift",
			"neo.haptic",
			"1.0.0",
			tmpDir,
		...
		expect(result.tarballPath).toContain("neo.haptic-1.0.0.tgz")
		expect(result.fileCount).toBeGreaterThan(0)
		expect(fs.existsSync(result.tarballPath)).toBe(true)

		// Verify tarball entries have package/ prefix
		const entries = []
		await tar.t({
		...
		expect(entries).toContain("package/Package.swift")
		expect(entries).toContain("package/lpm.config.json")
		expect(
			entries.some(e => e.includes("package/Sources/Haptic/Haptic.swift")),
		).toBe(true)
	})

	it("extracted with strip:1 produces correct paths", async () => {
		// Set up a minimal project
		fs.writeFileSync(path.join(tmpDir, "Package.swift"), "// swift")
		fs.writeFileSync(path.join(tmpDir, "lpm.config.json"), "{}")
		fs.mkdirSync(path.join(tmpDir, "Sources"), { recursive: true })
		fs.writeFileSync(path.join(tmpDir, "Sources", "Lib.swift"), "struct Lib {}")
		...
		// Extract with strip:1 (same as lpm add)
		const extractDir = path.join(tmpDir, "extracted")
		fs.mkdirSync(extractDir)
		await tar.x({ file: result.tarballPath, cwd: extractDir, strip: 1 })

		// Root-level files should be preserved
		expect(fs.existsSync(path.join(extractDir, "Package.swift"))).toBe(true)
		expect(fs.existsSync(path.join(extractDir, "lpm.config.json"))).toBe(true)
		expect(fs.existsSync(path.join(extractDir, "Sources", "Lib.swift"))).toBe(
			true,
		)
	})
})
