...
import { getRegistryUrl, setRefreshToken, setToken } from "../config.js"
...
export async function login() {
	...
	const server = http.createServer(async (req, res) => {
		...
		if (url.pathname === "/callback") {
			// Exchange code flow (Feature 42) — code is exchanged for token server-side
			const exchangeCode = url.searchParams.get("code")
			// Legacy fallback: direct token in URL (for older server versions)
			const directToken = url.searchParams.get("token")

			let token = directToken
			let refreshToken = null
			if (exchangeCode && !directToken) {
				try {
					const exchangeRes = await fetch(`${registryUrl}/api/cli/exchange`, {
						method: "POST",
					...
					if (exchangeRes.ok) {
						...
						token = data.token
						// Feature 44: store refresh token if server provides one
					...
					spinner.fail(`Token exchange failed: ${err.message}`)
			...
			if (token) {
				...
				// Store refresh token for session auth (Feature 44 Part B)
				if (refreshToken) {
...
<body>
  ...
  <script>
	...
	const fp = createHash("sha256")
		.update(`${hostname()}:${userInfo().username}:lpm-cli`)
		.digest("hex")
	const dn = encodeURIComponent(hostname())

	server.listen(0, async () => {
		const port = server.address().port
		const loginUrl = `${registryUrl}/cli/login?port=${port}&fp=${fp}&dn=${dn}`
