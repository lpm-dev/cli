...
`
	...
	const editorSetup = options?.editorSetup !== false
	...
	if (isJson) {
		...
		child.on("close", async code => {
			...
			if (fetchSkills) {
				const skillOpts = { json: true, editorSetup }
				if (explicitPackages) {
					for (const pkg of packages.filter(isLpmPackage)) {
						await skillsInstall(pkg, skillOpts)
					}
				} else {
					await skillsInstall(null, skillOpts)
				}
			}
		})
