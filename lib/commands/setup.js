...
/**
 * Configure CI/CD .npmrc for LPM packages.
 *
...
export async function setup(options) {
  ...
  p.intro(chalk.bgCyan(chalk.black(" lpm setup ci ")));
  ...
  if (options?.oidc) {
    ...
    try {
      ...
      log.warn(
        "Falling back to ${LPM_TOKEN} environment variable placeholder.",
      );
...
`.trim();
  ...
  log.info("For local development: Run `lpm setup local` to generate a token.");
  log.info("For CI/CD: Set the LPM_TOKEN environment variable.");
  log.info(
    "For CI/CD with OIDC: Run `lpm setup ci --oidc` (no secrets needed).",
  );
  console.log("");
