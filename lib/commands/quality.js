...
export async function quality(nameInput, options = {}) {
	...
	const spinner = options.json ? null : ora(`Fetching quality report for ${fullName}...`).start()
	...
	try {
		...
		if (data.score === null || data.score === undefined) {
			...
			console.log(chalk.dim('  Publish with lpm CLI v0.16+ to generate quality scores.'))
			console.log('')
			return
		}
		...
		console.log(`  ${chalk.bold('Quality Score:')} ${tierColor(`${data.score}/${data.maxScore}`)} ${tierColor(`(${tierLabel})`)}`)
		console.log('')

		// Category bars
		if (data.categories) {
			for (const [cat, { score: catScore, max }] of Object.entries(data.categories)) {
