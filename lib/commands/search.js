 ...
 * Search Command
 *
 * Search for packages in the LPM marketplace.
 *
 * Usage:
...
export async function search(query, options = {}) {
  ...
  try {
    ...
    // Calculate column widths (use new LPM format: @lpm.dev/owner.name)
    const maxNameLength = Math.max(
      ...packages.map(
        p => (p.owner ? `@lpm.dev/${p.owner}.${p.name}` : p.name).length,
      ),
      10,
    );

    for (const pkg of packages) {
      const fullName = pkg.owner
        ? `@lpm.dev/${pkg.owner}.${pkg.name}`
        : pkg.name;
      const paddedName = fullName.padEnd(maxNameLength);
      const version = chalk.dim(`v${pkg.version || 'unknown'}`.padEnd(12));
      const downloads = chalk.green(
        `↓ ${formatDownloads(pkg.downloads || 0)}`.padEnd(10),
      );
