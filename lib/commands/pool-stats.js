...
export async function poolStats(options = {}) {
	const spinner = options.json ? null : ora('Fetching pool statistics...').start()
	...
	try {
		...
		// Summary stats
		console.log(`  ${chalk.dim('Author Pool:')}    ${formatCents(data.authorPoolCents)}`)
		console.log(`  ${chalk.bold('Your Estimate:')}  ${chalk.green(formatCents(data.estimatedEarningsCents))}`)
		console.log('')

		if (packages.length === 0) {
			console.log(chalk.dim('  No pool packages found.'))
			console.log(chalk.dim('  Publish a package with Pool distribution to start earning.'))
			console.log('')
			return
		}
