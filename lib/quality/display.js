 ...
 *
 * @param {{ score: number, checks: Array, meta: object, warnings?: string[] }} result
 */
export function displayQualityReport({ score, checks, meta, warnings }) {
	const tierColor = TIER_COLORS[meta.tier] || chalk.white
	const tierLabel = TIER_LABELS[meta.tier] || meta.tier

	const hasServerOnly = checks.some(c => c.server_only)
	...
	const failed = checks.filter(c => !c.passed && !c.server_only)
	if (failed.length > 0) {
		console.log("")
		const tips = failed.slice(0, 3)
		for (const check of tips) {
			console.log(
				chalk.dim(`  Tip: ${getTip(check.id)} (+${check.max_points} points)`),
			)
		}
	}

	// Show warnings (e.g., stale skills)
	...
	if (failed.length > 0 && score < 90) {
		console.log(
			chalk.dim(
				"  Use the lpm-guide skill in your AI editor to improve quality and prepare for publish",
			),
		)
	}
