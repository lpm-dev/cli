 ...
 * @param {Array} [params.projectFiles] - Local project file list for test detection
...
export function runQualityChecks({
	...
	const results = checks.map(check => {
		...
		return {
			...
			...(result.server_only && { server_only: true }),
		}
	})

	// Compute category scores dynamically from actual check max_points
	const categories = {}
	for (const result of results) {
		...
		categories[result.category].max += result.max_points
	}
