...
const noLifecycleScripts = {
	...
	run: ({ packageJson }) => {
		...
		const found = lifecycleNames.filter(name => scripts[name])
		if (found.length === 0) {
			return {
				passed: true,
		...
		return {
			passed: false,
			detail: `Lifecycle scripts found: ${found.join(", ")}`,
		}
	},
}
...
const smallDeps = {
	...
	run: ({ packageJson }) => {
		...
		let points
		if (deps === 0) points = 3
		else if (deps <= 3) points = 2
		else if (deps <= 7) points = 1
		else points = 0
		return {
			passed: points > 0,
			points,
...
const hasTestFiles = {
	...
	max_points: 7,
	run: ({ projectFiles, files }) => {
		// Prefer projectFiles (local project scan) over files (tarball)
		// so authors don't need to ship test files in the package
		const fileList = projectFiles || files
...
const semverConsistency = {
	...
	run: ({ packageJson }) => {
		...
		return {
			...
			server_only: !isValid ? undefined : true,
		}
	},
}
