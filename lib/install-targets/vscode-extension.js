 ...
 */
export async function installVscodeExtension({ name, version, lpmConfig, extractDir, options }) {
	...
	if (!fs.existsSync(path.join(os.homedir(), '.vscode'))) {
		return {
			success: false,
			message: 'VS Code not detected. Install VS Code first (no ~/.vscode directory found).',
		}
	}
