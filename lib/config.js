...
/**
 * Store a refresh token in secure storage (Feature 44 Part B).
 * Uses the same secure-store as auth tokens with a "refresh:" prefix.
 * @param {string} refreshToken
 * @returns {Promise<void>}
 */
export async function setRefreshToken(refreshToken) {
	const registryUrl = getRegistryUrl()
	await secureSetToken(refreshToken, `refresh:${registryUrl}`)
}
...
export async function getRefreshToken() {
	const registryUrl = getRegistryUrl()
	return secureGetToken(`refresh:${registryUrl}`)
}
